...
public sealed class StorageAnalyticsService : IStorageAnalyticsService
{
    ...
    private static ChartDefinition BuildDriveCapacity(IReadOnlyList<VolumeInfo> volumes)
    {
        var points = volumes
            ...
            .ToList();
        return Chart("drive-capacity-rings", "Drive fullness", "Space", "Used vs free space by discovered drive.", VisualChartKind.Donut, points, insight: "Fuller drives deserve the first scan or cleanup review.");
    }
    ...
    private static ChartDefinition BuildTopFolderPareto(IReadOnlyList<FileSystemNode> nodes, long totalBytes)
    {
        ...
        var points = nodes
            ...
            .ToList();
        return Chart("top-folder-pareto", "Biggest folders", "Space", "Largest folders plus cumulative share of the drive.", VisualChartKind.RankBar, points, insight: "A few folders usually explain most user-visible pressure.");
    }
    ...
    private static ChartDefinition BuildCumulativeSizeCurve(IReadOnlyList<FileSystemNode> nodes, long totalBytes)
    {
        ...
        var points = nodes
            ...
            .ToList();
        return Chart("cumulative-size-curve", "Largest items curve", "Space", "How quickly large items account for the scan.", VisualChartKind.Curve, points, insight: "A steep early curve means targeted cleanup beats broad file hunting.");
    }
    ...
    private static ChartDefinition BuildFolderDepthHeatmap(IReadOnlyList<FileSystemNode> directories)
    {
        var cells = directories
            ...
            .ToList();
        return Chart("folder-depth-heatmap", "Folder depth by type", "Space", "Where large folders sit by depth and category.", VisualChartKind.Heatmap, cells: cells, insight: "Deep hot cells often point at generated caches or dependency folders.");
    }
    ...
    private static ChartDefinition BuildSubtreeEntropyMap(IReadOnlyList<FileSystemNode> directories, IReadOnlyList<FileSystemNode> nodes)
    {
        ...
        var cells = directories
            ...
            .ToList();
        return Chart("subtree-entropy-map", "Folder mix", "Space", "Balanced vs concentrated folders by child-size distribution.", VisualChartKind.Heatmap, cells: cells, insight: "A low mix means one child dominates; a high mix means broad cleanup needs category tools.");
    }
    ...
    private static ChartDefinition BuildExtensionRank(IReadOnlyList<FileSystemNode> files)
    {
        var points = files
            ...
            .Take(14)
            .ToList();
        return Chart("extension-rank", "File types by size", "File makeup", "File extensions ordered by physical size.", VisualChartKind.RankBar, points, insight: "Large extensions reveal whether pressure is media, installers, archives, or developer output.");
    }
    ...
    private static ChartDefinition BuildFileCountScatter(IReadOnlyList<FileSystemNode> directories)
    {
        var points = directories
            ...
            .ToList();
        return Chart("file-count-scatter", "Many files vs big folders", "File makeup", "Folders plotted by file count and megabytes.", VisualChartKind.Scatter, points, insight: "High-count, low-size folders are build/cache cleanup targets.");
    }
    ...
    private static ChartDefinition BuildTinyFileDensity(IReadOnlyList<FileSystemNode> files)
    {
        var points = files
            ...
            .ToList();
        return Chart("tiny-file-density", "Small-file pressure", "File makeup", "Categories with many small files.", VisualChartKind.Scatter, points, insight: "Small-file pressure is a performance signal, not just a space signal.");
    }
    ...
    private static ChartDefinition BuildLargestFileBubbles(IReadOnlyList<FileSystemNode> files)
    {
        var points = files
            ...
            .ToList();
        return Chart("largest-file-bubbles", "Largest files", "File makeup", "Individual heavy files sized as bubbles.", VisualChartKind.BubblePack, points, insight: "Single-file triage is fastest for VHDX, ISO, media, backup, and dump files.");
    }
    ...
    private static ChartDefinition BuildModifiedAgeTimeline(IReadOnlyList<FileSystemNode> files)
    {
        ...
        var points = files
            ...
            .OrderBy(p => p.X)
            .ToList();
        return Chart("modified-age-timeline", "File age", "Time", "Bytes grouped by file age.", VisualChartKind.Timeline, points, insight: "Old large bytes are good archive/move candidates when ownership is clear.");
    }
    ...
    private static ChartDefinition BuildCalendarHeatmap(IReadOnlyList<FileSystemNode> files)
    {
        var cells = files
            ...
            .ThenBy(c => c.Column)
            .ToList();
        return Chart("modified-calendar-heatmap", "Activity calendar", "Time", "Activity intensity by week and day.", VisualChartKind.CalendarHeatmap, cells: cells, insight: "Clusters can identify imports, downloads, build bursts, or backup days.");
    }
    ...
    private static ChartDefinition BuildStaleDecayCurve(IReadOnlyList<FileSystemNode> files)
    {
        ...
        var points = buckets
            ...
            .ToList();
        return Chart("stale-decay-curve", "Old file curve", "Time", "Share of bytes older than each age threshold.", VisualChartKind.Curve, points, insight: "A high 365-day value means archive policy can matter more than cache clearing.");
    }
    ...
    private static ChartDefinition BuildRecentGrowthTimeline(IReadOnlyList<ChangeRecord> changes)
    {
        var points = changes
            ...
            .ToList();
        return Chart("recent-growth-timeline", "Recent growth", "Time", "Snapshot changes in detection order.", VisualChartKind.Timeline, points, insight: "Growth spikes connect user-visible pressure to recent tools or updates.");
    }
    ...
    private static ChartDefinition BuildDeltaWaterfall(IReadOnlyList<ChangeRecord> changes)
    {
        var points = changes
            ...
            .ToList();
        return Chart("snapshot-delta-waterfall", "Biggest changes", "Time", "Largest growth and shrink events since the previous scan.", VisualChartKind.Waterfall, points, insight: "This separates real growth from cleanup wins.");
    }
    ...
    private static ChartDefinition BuildReclaimableFunnel(IReadOnlyList<CleanupFinding> findings)
    {
        var points = findings
            ...
            .OrderByDescending(p => p.SizeBytes)
            .ToList();
        return Chart("reclaimable-safety-funnel", "Cleanup by safety", "Cleanup", "Cleanup potential grouped by guardrail lane.", VisualChartKind.Funnel, points, insight: "Safe is actionable, Review needs human context, system lanes stay guarded.");
    }
    ...
    private static ChartDefinition BuildCleanupEffortValue(IReadOnlyList<CleanupFinding> findings)
    {
        var points = findings
            ...
            .ToList();
        return Chart("cleanup-effort-value", "Cleanup value", "Cleanup", "Bigger reclaim vs review effort.", VisualChartKind.Scatter, points, insight: "Best candidates sit high and left: large, safe, and backed by strong evidence.");
    }
    ...
    private static ChartDefinition BuildSafetyStack(IReadOnlyList<CleanupFinding> findings)
    {
        var series = findings
            ...
            .Take(8)
            .ToList();
        return Chart("safe-review-stacked-bars", "Cleanup lanes by type", "Cleanup", "Cleanup lanes by finding category.", VisualChartKind.StackedBar, series: series, insight: "Developer artifacts often split between safe generated caches and review-heavy dependencies.");
    }
    ...
    private static ChartDefinition BuildBlockedRiskMap(IReadOnlyList<CleanupFinding> findings)
    {
        var cells = findings
            ...
            .ToList();
        return Chart("blocked-system-risk-map", "Protected system areas", "Cleanup", "Where cleanup guardrails prevent unsafe actions.", VisualChartKind.Matrix, cells: cells, insight: "Blocked cells are explanations, not action buttons.");
    }
    ...
    private static ChartDefinition BuildOwnershipFlow(IReadOnlyList<StorageRelationship> relationships, IReadOnlyList<FileSystemNode> nodes)
    {
        ...
        var flows = relationships
            ...
            .Take(18)
            .ToList();
        return Chart("app-ownership-sankey", "App storage links", "Ownership", "Evidence-backed owners linked to storage roots.", VisualChartKind.RelationshipFlow, flows: flows, insight: "Ownership turns unknown folders into app-specific decisions.");
    }
    ...
    private static ChartDefinition BuildCacheOwnershipMatrix(IReadOnlyList<StorageRelationship> relationships, IReadOnlyList<CleanupFinding> findings)
    {
        ...
        var cells = relationships
            ...
            .ToList();
        return Chart("cache-ownership-matrix", "Cache owners", "Ownership", "Owners crossed with relationship type.", VisualChartKind.Matrix, cells: cells, insight: "Cache ownership helps distinguish disposable data from active state.");
    }
    ...
    private static ChartDefinition BuildDeveloperDependencyFlow(IReadOnlyList<StorageRelationship> relationships, IReadOnlyList<FileSystemNode> nodes)
    {
        ...
        var flows = relationships
            ...
            .Take(18)
            .ToList();
        return Chart("dev-artifact-dependency-graph", "Project dependency links", "Ownership", "Generated folders connected to package manifests and build tools.", VisualChartKind.RelationshipFlow, flows: flows, insight: "Generated dependencies are removable only when the project can reinstall them.");
    }
    ...
    private static ChartDefinition BuildCloudLocalStatus(IReadOnlyList<FileSystemNode> nodes, IReadOnlyList<StorageRelationship> relationships)
    {
        ...
        return Chart("cloud-local-status-bars", "Cloud vs local", "Ownership", "Separates local-only bytes from cloud-linked roots.", VisualChartKind.StackedBar, points, insight: "Cloud-linked storage needs sync risk warnings before cleanup.");
    }
    ...
    private static ChartDefinition BuildPackageManagerFootprint(IReadOnlyList<FileSystemNode> nodes, IReadOnlyList<CleanupFinding> findings)
    {
        var candidates = nodes
            ...
            .ToList();
        return Chart("package-manager-footprint", "Package caches", "System/dev", "npm, NuGet, Gradle, Python, Docker, and similar storage roots.", VisualChartKind.RankBar, candidates, insight: "Package caches are high-yield but can slow the next restore/build.");
    }
    ...
    private static ChartDefinition BuildSystemDevRoots(IReadOnlyList<FileSystemNode> nodes)
    {
        var points = nodes
            ...
            .ToList();
        return Chart("docker-wsl-storage-roots", "Docker and WSL storage", "System/dev", "Heavy virtualized or system-managed developer roots.", VisualChartKind.BubblePack, points, insight: "Virtual disks and image stores can dominate space but require tool-aware cleanup.");
    }
    ...
    private static ChartDefinition BuildDependencyCacheRadar(IReadOnlyList<FileSystemNode> nodes)
    {
        ...
        var points = categories
            ...
            .ToList();
        return Chart("dependency-cache-comparison", "Developer cache comparison", "System/dev", "Developer cache families normalized onto one radar.", VisualChartKind.Radar, points, insight: "Spikes show which ecosystem is responsible for repeated build storage.");
    }
    ...
    private static ChartDefinition BuildInstallerRiskRadar(IReadOnlyList<CleanupFinding> findings, IReadOnlyList<FileSystemNode> nodes)
    {
        ...
        return Chart("installer-cache-risk-radar", "Installer and update caches", "System/dev", "System and installer storage areas normalized by risk and size.", VisualChartKind.Radar, points, insight: "Installer and system caches are usually review or official-cleanup lanes.");
    }
    ...
    private static IReadOnlyList<TutorialStep> BuildTutorials()
    {
        ...
        [
            ...
            Tutorial("charts", "Use Charts", "Find the strongest storage story", "Start with Best insights, then expand More charts when you need a deeper explanation.", "Click a chart item to sync the selected path when the chart has a node-backed point.", "Charts explain; cleanup still happens through review lanes."),
            ...
            Tutorial("privacy", "Check privacy posture", "Confirm everything stays local", "Open Privacy & Safety Center to review telemetry status, local database location, diagnostics export behavior, and blocked cleanup paths.", "Use redacted diagnostics exports when sharing bug reports.", "No external advisor, telemetry, or credential integration is used."),
